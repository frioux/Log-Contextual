...
for my $level (@levels) {
   ...
   my @sfoo =
     main->can("Dslog_$level")->("Look ma, data: ", qw{frew bar baz});
   ok(
   ...
   my $sbar =
     main->can("DslogS_$level")
     ->("Look ma, data: ", [qw{frew bar baz}]);
   ok(
   ...
   @sfoo = main->can("Dslog_$level")->("nothing: ", ());
   ok(eq_array(\@sfoo, []), "Dslog_$level passes nothing through correctly");
   is($var, "[$level] nothing: ()\n", "Output for Dslog_$level is correct");
   ...
   @sfoo = main->can("DslogS_$level")->("nothing: ", ());
   ok(eq_array(\@sfoo, []), "Dslog_$level passes nothing through correctly");
   is($var, "[$level] nothing: undef\n", "Output for DslogS_$level is correct");
}
